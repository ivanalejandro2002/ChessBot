...
using namespace std;
void impTablero(SDL_Renderer *renderer,SDL_Event &event,chessPosition &positionToPrint){
    ...
    for(int i=1;i<=8;i++){
        for(int j=1;j<=8;j++){
            if(pieceTranslator.count(positionToPrint.position[i][j])){
                pieza.y = SIDECELL*(i-1)+54;
                pieza.x = SIDECELL*(j-1)+54;
                SDL_RenderCopy(renderer,pieceSprites[pieceTranslator[positionToPrint.position[i][j]]],NULL,&pieza);
            }
        }
    }
